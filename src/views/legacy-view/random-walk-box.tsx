...
function drawChart(containerId: string, domain: [number, number], data: Viz.RandomWalk.Result['data'], i = 0) {
    ...
    const svgID = `chart_randomWalk_${i}`;
    ...
    const xAttr = (d: Viz.RandomWalk.Value) => {
        return xScale(d[0]);
    };
    const yAttr = (d: Viz.RandomWalk.Value) => {
        return yScale(d[1]);
    };

    const bars = container
        .selectAll('.bar')
        .data<Viz.RandomWalk.Value>(data.values)
        .enter()
        .append('rect')
        .classed('bar', true)
        .attr('width', 1)
...
function drawCharts(containerId: string) {
    (() => {
        const { domain, data } = viz.getRandomWalk({
            samples: 1000,
            iterations: 100,
            initialValue: 0,
            step: 1,
        ...
        drawChart(containerId, domain, data, 0);
    })();
    ...
    (() => {
        ...
        drawChart(containerId, domain, data, 1);
    })();
    ...
    (() => {
        ...
        drawChart(containerId, domain, data, 2);
    })();
    ...
    (() => {
        ...
        drawChart(containerId, domain, data, 3);
    })();
}

export function RandomWalkBox() {
    const wrapperId = 'random-walk-box';
